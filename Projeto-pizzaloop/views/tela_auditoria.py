...
FILTROS_CARGO = {
    ...
    "Func. + Entreg.": ["funcionario", "entregador"],
}
...
def renderizar_auditoria(frame_conteudo, janela_raiz, paleta_cores):
    ...
    seletor = ctk.CTkSegmentedButton(
        barra, values=list(FILTROS_CARGO.keys()),
        variable=filtro_var,
        selected_color=COR_PRIMARIA, selected_hover_color=COR_PRIMARIA,
        command=lambda _: _desenhar_tabela()
    )
    seletor.pack(side="right")

    ctk.CTkFrame(area, height=1, fg_color=COR_BORDA).pack(fill="x", padx=28, pady=(4, 14))

    container_tabela = ctk.CTkFrame(area, fg_color="transparent")
    container_tabela.pack(fill="x", padx=28, pady=(0, 28))

    def _desenhar_tabela():
        for w in container_tabela.winfo_children():
            w.destroy()

        cargos = FILTROS_CARGO[filtro_var.get()]
        try:
            registros = controlador.listar_historico(limite=200, cargos=cargos)
        except Exception:
            registros = []

        card_tabela = ctk.CTkFrame(container_tabela, fg_color=COR_CARD, corner_radius=14, border_width=1, border_color=COR_BORDA)
        card_tabela.pack(fill="x")
